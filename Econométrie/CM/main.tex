...
\textbf{Statistique de test}
\begin{equation*}
		t_c = \frac{\hat{Y}_0 - (\upalpha + \upbeta X_0)}{\hat{\upsigma}_\varepsilon \sqrt{1 + \frac{1}{n} + \frac{(X_0 - \overline{X})^2}{\sum x_t^2}}} \sim T(n-2)
\end{equation*}
\textbf{Intervalle d'acceptation}
\begin{equation*}
	\begin{split}	
		1 - \upalpha &= Prob\left[\studn < T(n-2) <\studp \right] \\
		&= Prob\left[\studn < \frac{\hat{Y}_0 - (\upalpha + \upbeta X_0)}{\hat{\upsigma}_\varepsilon \sqrt{1 + \frac{1}{n} + \frac{(X_0 - \overline{X})^2}{\sum x_t^2}}} <\studp \right] \\
